...
describe('EmpresasView', () => {
  ...
  it('shows validation errors instead of submitting an invalid empresa', async () => {
    ...
    expect(body.text()).toContain('El nombre debe tener al menos 2 caracteres.')
  })
  ...
  it('creates an empresa when the form is valid', async () => {
    ...
    await flushPromises()
    const body = new DOMWrapper(document.body)
    await body.find('input[type="text"]').setValue('Empresa Nueva')
    await body.find('input[type="number"]').setValue(30000)
    await body.find('form').trigger('submit')
    await flushPromises()
